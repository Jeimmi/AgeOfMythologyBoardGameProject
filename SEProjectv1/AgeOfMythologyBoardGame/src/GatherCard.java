...
public class GatherCard extends Card{
	...
	public void gatherByTerrain(Game game, ProductionTile.Terrain terrainType){
		for(int i = 0; i < game.activePlayer.production.size(); i++){
			if(game.activePlayer.production.get(i).type == terrainType){
				for(int j = 0; j < 4; j++){
					if(game.bank[j] < game.activePlayer.production.get(i).resource[j]){
						game.activePlayer.wallet[j] += game.bank[j];
						game.bank[j] = 0;
					}
					else{
						game.activePlayer.wallet[j] +=  
	...
	public void execute(Game game){
		...
		for(int i = 0; i < game.activePlayer.city.size(); i++){
			...
			if(bonusIndex < 5){
				if(game.bank[bonusIndex] > 1){
					game.bank[bonusIndex] -= 2;
					game.activePlayer.wallet[bonusIndex]  +=2;
				}
				else if(game.bank[bonusIndex] == 1){
					game.bank[bonusIndex] -= 1;
					game.activePlayer.wallet[bonusIndex] +=1;
				}
			}
		...
		for(int i = 0; (i < numberOfVillagers) && 
				(i < game.activePlayer.production.size()); i++){
			ProductionTile temp;
